...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Portfolio
      .findOne(_.pick(params, _.keys(Portfolio.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count portfolios.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Portfolio.create(data);
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Portfolio.associations.map(a => a.alias));
    const data = _.omit(values, Portfolio.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Portfolio.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Portfolio.associations.map(async association => {
